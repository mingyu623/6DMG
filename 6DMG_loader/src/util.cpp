...
int GestToMat(char* file, Gesture& g)
{
	...
	int num = g.data.size();
	pa  = mxCreateNumericMatrix(N_ELEM, num, mxSINGLE_CLASS, mxREAL); // (row, col) or (tuple, sample #)
...
{	
	...
	vector<float> psis, thetas, phis; // euler angles in Z(roll), X(pitch), Y(yaw) respectively		
	if (HTK_EXP & (HTK_ORI_LR | HTK_EA_PITCH)) // special case for left-to-right ori
	{		
		// quat -> euler
		for (unsigned int i=0; i<g.data.size(); i++){
	...
	for (unsigned int i=0; i<g.data.size(); i++)
	{
		...
		// TODO: implement operator overloads for my structs		
		...
		if (HTK_EXP & (HTK_ORI | HTK_ORI_LR)){
			...
			q = quatScale(q, scale_ori);  // scale the rotation angle will cause HERest fail for motion words?
...
float normalizePOS(Gesture& g)
{
	...
	float diff;
	if (HTK_EXP & HTK_POS_LR)		// only use the y position
		diff = diff_y;
...
void leastSquaresFit(vector<float>& y, float& a, float& b)
{
	float sum_x = 0;
	float sum_y = 0;
	float sum_x_sq = 0;
...
{
	Euler e;
	e.psi   = atan2( 2*(q.w*q.z-q.x*q.y), q.w*q.w-q.x*q.x+q.y*q.y-q.z*q.z );
	e.theta = asin ( 2*(q.w*q.x+q.y*q.z) );
	e.phi   = atan2( 2*(q.w*q.y-q.x*q.z), q.w*q.w-q.x*q.x-q.y*q.y+q.z*q.z );
	return e;
}
